...
import ru.jts.data.common.Point3WithHeading;
import ru.jts.data.common.Point4;
import ru.jts.data.common.Territory;
...
public class Residence {
    ...
    public GetType get_type; // Тип получения, осада, битва с npc (для осадных КХ), аукцион
    ...
    @ObjectArray
    public Point3[] owner_restart_point_list; // Возможно, cписок точек, в которых появляются владельцы резиденции после смерти
    @ObjectArray
    ...
    @ObjectArray
    public ResidenceGuardMap[] residence_guard_mapping; // Неизвестно, какие-то нпс, есть зависимость от выигравшей печати

    @ObjectArray
    public ResidenceNpc[] owning_npc_list; // Список каких-то НПС
    @StringArray
    public String[] boss_npc_list; // Список каких-то НПС, присутствует у devastated_castle
    ...
    @EnumValue
    public ResidenceState residence_state; // флаг, означающий состояние резиденции. (в офф скриптах везде initial)
    @ObjectArray
    ...
    @IntValue
    public int agit_parent_castleid; // ID резиденции, к которой привязана данная резиденция (-1 если непривязана ни к чему, 0 для Замка Аден)
    @IntValue
    ...
    @ObjectArray
    public Point3WithHeading[] battle_royal_flag_point_list; // Список каких-то точек у beast_farm_guildhall, и bandits'_stronghold
    @ObjectArray
    public Point3WithHeading[] final_flag_point_list; // Список каких-то точек у beast_farm_guildhall, и bandits'_stronghold
    @StringArray
    public String[] final_open_door_list; // Список дверей, активен у beast_farm_guildhall, и bandits'_stronghold
    ...
    @StringArray
    public String[] agit_delete_item_list; // Неизвестно, активно у rainbow_spring

    @StringArray
    public String[] residence_skill_list; // Список скилов, активно у земель, фортов и замков
    ...
    public static class ResidenceGuardMap {
        ...
        @EnumValue(withoutName = true)
        private MoveType move_type; // Тип передвижения, двигается или стоит на месте
        ...
        public SSQType[] ssq_type; // Выигравшие печати, при которых активны данные гварды

        public static enum GuardType {
            sword, bow, pole, cleric, wizard, teleporter
        }

        public static enum SSQType {
            ssq_normal, ssq_dawn, ssq_twilight
        }
