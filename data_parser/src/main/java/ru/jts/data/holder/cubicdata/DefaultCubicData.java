...
public class DefaultCubicData {
    ...
    @IntValue
    public int slot; // какой-то слот, не может быть одновременно два помощника с одинаковыими слотами.
    @IntValue
    public int duration = -1; // Продолжительность (по умолчанию -1, без времени)

    @IntValue
    public int delay; // Какая-то задержка, возможно между действиями кубика\агатиона
    @IntValue
    public int max_count = 0; // какое-то количество. Присутствует там, где delay > 0
    @IntValue
    ...
    public CubicDataTargetType target_type; // Тип цели, если by_skill - определяется скилом
    ...
    public static class CubicDataTargetType {
        public TargetType type; // target - кубик действует по цели владельца, heal - лечит самона/владельца, by_skill - в зависимости от скила

        // Только для type = heal, указывает, какой скил будет использоваться
        // Например в массиве (90;60;30;0). 100-90 - не используется, 90-60 - первый скилл, 60-30 - второй скил, 0-30 - третий скил
    ...
    public static class CubicDataSkill {
        // Может отсутствовать
        public int skillChance; // Шанс того, что помощник будет использовать именно этот скилл, отсутствует для скила типа TargetType.heal
        ...
        // Может отсутствовать
        // Если CubicData.target_type = by_skill, то для определения цели используются эти параметры
