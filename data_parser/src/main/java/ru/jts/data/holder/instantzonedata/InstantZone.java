...
import static ru.jts.data.holder.InstantZoneDataHolder.EntranceCondObjectFactory;
...
public class InstantZone {
    ...
    @IntValue
    public int max_channel; // Максимальное кол-во одновременно активных зон данного типа
    @IntValue
    public int min_user = 1; // Минимальное кол-во игроков в одной инстанс-зоне данного типа, есть не у всех
    @IntValue
    public int max_user; // Максимальное кол-во игроков в одной инстанс-зоне данного типа
    @IntValue
    public int duration; // Продолжительность (в минутах)
    @IntValue
    public int time_limit; // Время "отката" инстанс зоны (в минутах).
    @ObjectValue
    public ResetBindig reset_binding; // Время сброса инстанс зоны для всех игроков, может быть null
    ...
    @IntValue
    public int delete_no_user; // Время, через которое будет удалена пустая инстанс зона

    @ObjectArray
    public ItemName_Count[] required_item = new ItemName_Count[0]; // Предметы, которые должны быть в инвентаре, при входе в инстанс зону (не известно, всегда, или только при первом входе)
    @ObjectArray
    public ItemName_Count[] deleted_item = new ItemName_Count[0]; // Предметы, которые удаляются при входе в инстанс зону (не известно, всегда, или только при первом входе)

    @IntValue
    public int remove_buff; // Удалять ли бафф с персонажа. 1 - удалять, 0 - не удалять
    ...
    public Point3[] end_poses = new Point3[0]; // Список точек - конечных позиций, для end_pos_type = origin - список пуст
    ...
    public Point3[] disorder_end_poses = new Point3[0]; // Список точек - конечных позиций

    @StringArray
    public String[] door_list = new String[0]; // Список дверей, привязанных к инстанс зоне
    @StringArray
    public String[] area_list = new String[0]; // Список областей, привязанных к инстанс зоне

    @EnumValue
    public YesNo spawn_npc; // Спаунить ли NPС при создании зоны (список NPC находится в npcpos.txt)
    @EnumValue
    ...
    public static class ResetBindig {
        ...
        public int[] day_of_week; // Дни недели, по которым выполняется сброс (0 = воскресенье, 1 = понедельник, 2= вторник...)
    }
}
