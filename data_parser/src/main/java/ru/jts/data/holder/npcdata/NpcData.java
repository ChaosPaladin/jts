...
package ru.jts.data.holder.npcdata;

import ru.jts.annotations.data.array.DoubleArray;
...
import static ru.jts.data.holder.NpcDataHolder.NpcAIObjectFactory;
...
public class NpcData {
    ...
    @IntValue
    public int corpse_time; // Время, в течении которого труп NPC будет лежать на земле
    @IntValue
    public int no_sleep_mode; // Время, в течении которого труп NPC будет лежать на земле
    @IntValue
    ...
    @ObjectValue
    public EventFlag event_flag; // Всегда имеет значение, (0 либо 1) Не может быть Null

    @DoubleValue
    public double acquire_exp_rate; // Рейт EXP, используемый при расчете опыта за убийство монстра
    @IntValue
    ...
    @ObjectArray
    public DropInfo[] corpse_make_list = new DropInfo[0]; // Спойл с монстра (Не групповой расчет)
    @ObjectArray
    public DropInfo[] additional_make_list = new DropInfo[0]; // Обычный дроп с монстра (Не групповой расчет шанса)
    @ObjectArray
    public DropGroup[] additional_make_multi_list = new DropGroup[0]; // Обычный дроп с монстра (Групповой расчет шанса)
    @ObjectArray
    public DropGroup[] ex_item_drop_list = new DropGroup[0]; // Дроп Herb'ов (Групповой расчет)

    @IntValue
    public int fake_class_id; // если != -1, то в пакете NpcInfo вместо npc_class_id должен использоваться этот class_id
    ...
    public static enum NpcRace {
        fairy, humanoid, animal, undead, beast,
        plant, bug, construct, elemental, demonic,
        giant, dragon, divine, human, kamael, elf, etc,
        dwarf, mercenary, orc, darkelf, castle_guard,
        siege_weapon
    }
